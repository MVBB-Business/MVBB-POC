 ...
 *
 * @supabase/supabase-js's generics require each table to provide Row,
 * Insert, Update, and Relationships (its GenericTable constraint) — a
 * table missing any of those silently resolves query results to `never`
 * instead of a real type, with no error at the call site. TableDef fills
 * in reasonable Insert/Update/Relationships defaults from just a Row type
 * so that mistake can't happen here again.
 */
type TableDef<Row> = {
  Row: Row;
  Insert: Partial<Row>;
...
export interface Database {
  public: {
    ...
    // supabase-js's GenericSchema constraint requires all of these keys to
    // be present (even empty) to resolve query result types correctly —
    // omitting them silently collapses every query's result to `never`
    // instead of raising a type error at the call site.
